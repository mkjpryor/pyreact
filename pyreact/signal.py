...
class Computed(Signal, Reactor):
    ...
    def __init__(self, calc):
        self.__calc = calc
        # State is a tuple of (level, value, error)
        #   level should be an int which is >= 0
        #   value can be anything
        #   error can be Exception or None
    ...
    @property
    def now(self):
        # If the state is an error, raise it
        _, value, error = self.__state
        if error:
            raise error
        else:
            return value
    ...
    def __recalculate(self):
        ...
        try:
            ...
            tracking.end()
        return max(levels) + 1 if levels else 0, result, error
